...
namespace AutoFixture.Xunit2.Internal
{
    ...
    internal sealed class InlineTestCaseSource : ITestCaseSource
    {
        ...
        public IEnumerable<IEnumerable<object>> GetTestCases(MethodInfo method)
        {
            ...
            return GetTestCases(method, this.values);

            static IEnumerable<object[]> GetTestCases(MethodBase method, object[] values)
            {
                var parameters = method.GetParameters();
                var inlineArgumentCount = Math.Min(parameters.Length, values.Length);

                var span = new ReadOnlySpan<object>(values);
                yield return span.Slice(0, inlineArgumentCount).ToArray();
            }
        }
    }
}