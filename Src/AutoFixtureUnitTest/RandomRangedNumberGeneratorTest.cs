...
namespace Ploeh.AutoFixtureUnitTest
{
    public class RandomRangedNumberGeneratorTest
    {
        ...
        public void CreateReturnsRandomDifferentValuesForRequestsOfSameTypeAndDifferentMaximums
            (int request1Min, int request1Max,int request2Min, int request2Max, int request3Min, int request3Max)
        {
            // Fixture setup
            ...
            // Verify that the range of values generated by the sut for a request match 
            // the range
            Assert.True(results[request1].Count() == request1Count);
            Assert.True(results[request2].Count() == request2Count);
            Assert.True(results[request3].Count() == request3Count);
            Assert.True(Enumerable.Range(request1Min, request1Count).Intersect(results[request1]).Count() == request1Count);
            Assert.True(Enumerable.Range(request2Min, request2Count).Intersect(results[request2]).Count() == request2Count);
            Assert.True(Enumerable.Range(request3Min, request3Count).Intersect(results[request3]).Count() == request3Count);       
        }

        [Theory]
        [InlineData(0,2,-1,2,-2,2)]
        public void CreateReturnsRandomDifferentValuesForRequestsOfSameTypeAndDifferentMinimums(int request1Min, int request1Max,
            int request2Min, int request2Max, int request3Min, int request3Max)
        {
            // Fixture setup            
            int request1Count = request1Max - request1Min + 1;
            int request2Count = request2Max - request2Min + 1;
        ...
        [Theory]
        [InlineData(new int[] { 0, 500 }, 100)]
        public void CreateReturnsUniqueNumbersOnMultipleCallAsynchronously(int[] limits, int timesToTry)
        {  
            // Fixture setup           
           ...
           while (tryCount++<timesToTry)
           {
               ...
               var sut = new RandomRangedNumberGenerator();
                ...
                for (int i = 0; i < iterations; i++)
                {
                    ...
                    {
                        var request = new RangedNumberRequest(typeof(int), min, max);
